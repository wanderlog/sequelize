...
class ConnectionManager {
  ...
  async getConnection(options) {
    ...
    if (this.trackConnectionUsage) {
      ...
      result._query = void 0;
      result._queryTime = void 0;
    }
  ...
  _getPoolDiagnostics() {
    ...
    const processPool = (pool, poolName) => {
      ...
      pool._inUseObjects.forEach((wrappedConnection, index) => {
        ...
        if (!connection)
          return;
        const heldTime = connection._acquireTime ? (now - connection._acquireTime) / 1e3 : 0;
        const queryTime = connection._queryTime ? (now - connection._queryTime) / 1e3 : null;
        ...
        if (connection._query) {
          const query = connection._query.length > 100 ? `${connection._query.substring(0, 100)}...` : connection._query;
    ...
    const totalConnections = this.pool.read && this.pool.write ? (((_a = this.pool.read._inUseObjects) == null ? void 0 : _a.length) || 0) + (((_b = this.pool.write._inUseObjects) == null ? void 0 : _b.length) || 0) : ((_c = this.pool._inUseObjects) == null ? void 0 : _c.length) || 0;
