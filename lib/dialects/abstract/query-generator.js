...
class AbstractQueryGenerator {
    ...
    if (Array.isArray(collection)) {
      collection.forEach((item2, index) => {
        ...
        if (typeof item2 === "string") {
          ...
          } else if (previousModel && (0, import_model_utils.isModelStatic)(previousModel)) {
  ...
  selectQuery(tableName, options, model) {
    ...
    if (subQuery) {
      subQueryItems.push(this.selectFromTableFragment(options, mainTable.model, attributes.subQuery, mainTable.quotedName, mainTable.as), subJoinQueries.join(""));
    } else {
    ...
    if (options.lock && this._dialect.supports.lock) {
      ...
      if (this._dialect.supports.lockOf && options.lock.of && (0, import_model_utils.isModelStatic)(options.lock.of)) {
  ...
  generateInclude(include, parentTableName, topLevelInfo) {
    ...
    if (topLevelInfo.options.includeIgnoreAttributes !== false) {
      ...
      const includeAttributes = include.attributes.map((attr) => {
        ...
        if (attr instanceof Utils.Cast || attr instanceof Utils.Fn) {
          throw new TypeError("Tried to select attributes using Sequelize.cast or Sequelize.fn without specifying an alias for the result, during eager loading. This means the attribute will not be added to the returned instance");
        }
        let prefix;
        if (verbatim === true) {
          prefix = attr;
    ...
    if (include.subQuery && topLevelInfo.subQuery) {
      if (requiredMismatch && subChildIncludes.length > 0) {
        joinQueries.subQuery.push(` ${joinQuery.join} ( ${joinQuery.body}${subChildIncludes.join("")} )${indexHintsSubstring} ON ${joinQuery.condition}`);
      } else {
        joinQueries.subQuery.push(` ${joinQuery.join} ${joinQuery.body}${indexHintsSubstring} ON ${joinQuery.condition}`);
      ...
      if (requiredMismatch && mainChildIncludes.length > 0) {
        joinQueries.mainQuery.push(` ${joinQuery.join} ( ${joinQuery.body}${mainChildIncludes.join("")} )${indexHintsSubstring} ON ${joinQuery.condition}`);
      } else {
        joinQueries.mainQuery.push(` ${joinQuery.join} ${joinQuery.body}${indexHintsSubstring} ON ${joinQuery.condition}`);
  ...
  _whereBind(binding, key, value, options) {
    ...
    return value.length > 0 ? `(${value.join(binding)})` : void 0;
  }
